...
if __name__ == '__main__':
    ...
    try:
        ...
        status = "successful"
        process_record = 0
        print(f'function : {function_name} run succssfully ')

    except Exception as e:
        print(f'function : {function_name} run fail {e} ')
        end_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        status = "fail"
        process_record = 0
    ...
    ########################################### function 4: extractRequiredDataAndFlatten #############################################################################################################

    ## information collect for audit log regarding extract data function
    try:
        function_name = "4_extractRequiredDataAndFlatten"
        start_time = datetime.now().strftime('%Y%m%d_%H%M%S')

        ## call extractRequiredData function for fetch required data
        reuired_data_lst_of_dic = util_obj.extractRequiredDataAndFlatten(json_data)
...
########################################### function 6: applyTransformation #############################################################################################################

    ## information collect for audit log regarding extract data function
    try:
        function_name = "6_applyTransformation"
        start_time = datetime.now().strftime('%Y%m%d_%H%M%S')

        ## call flattenData function for flattening the data
        flatten_data_df = util_obj.applyTransformation(earthquake_dataframe)
